import db from '@models';
import { createCrudService } from '../common/crud-service';
import { TenantRepository } from '../common/tenant-repository';
import { Page, Pagination } from '../common/pagination';
import { BadRequestError } from '../utils/errors';

const service = createCrudService<any>(db.SmallGroup, 'Small group', {
  order: [['name', 'ASC'], ['id', 'ASC']]
});

const memberships = new TenantRepository<any>(db.SmallGroupMember, 'Small group membership');
const members = new TenantRepository<any>(db.Member, 'Member');

export const repository = service.repository;

export const createSmallGroup = service.create;
export const getAllSmallGroups = service.list;
export const getSmallGroupById = service.findById;
export const updateSmallGroup = service.update;
export const deleteSmallGroup = service.remove;

async function assertBothInChurch(groupId: number, memberId: number): Promise<void> {
  await service.findByIdOrFail(groupId);
  if (!(await members.exists({ id: memberId }))) {
    throw new BadRequestError('That member does not belong to this church.');
  }
}

export const addMemberToSmallGroup = async (
  groupId: number,
  memberId: number,
  attributes: Record<string, unknown> = {}
) => {
  await assertBothInChurch(groupId, memberId);

  const existing = await memberships.findOne({ smallGroupId: groupId, memberId });
  if (existing) {
    throw new BadRequestError('That member is already in this group.');
  }

  return memberships.create({ ...attributes, smallGroupId: groupId, memberId });
};

export const removeMemberFromSmallGroup = async (
  groupId: number,
  memberId: number
): Promise<void> => {
  await service.findByIdOrFail(groupId);

  const membership = await memberships.findOne({ smallGroupId: groupId, memberId });
  if (!membership) {
    throw new BadRequestError('That member is not in this group.');
  }

  await membership.destroy();
};

export const getMembersOfSmallGroup = async (
  groupId: number,
  pagination: Pagination
): Promise<Page<any>> => {
  await service.findByIdOrFail(groupId);

  return memberships.list({
    pagination,
    where: { smallGroupId: groupId },
    include: [{ model: db.Member, as: 'member' }],
    order: [['memberId', 'ASC']]
  });
};

export default service;
